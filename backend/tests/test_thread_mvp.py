...
import pytest
from pythia.application.ai_providers.groq_client import GroqClient, GroqRateLimiter
...
async def test_confidence_gate():
    ...
    signal_low = TradingSignal(
        action="BUY", confidence=0.4, pair="BTC/USDT", reason="Low conf test"
    )
    assert signal_low.action == "HOLD", "Confidence gate failed: 0.4 should force HOLD"
    signal_high = TradingSignal(
        action="BUY", confidence=0.7, pair="BTC/USDT", reason="High conf test"
    )
...
async def test_groq_rate_limit():
    ...
    limiter = GroqRateLimiter(delay_seconds=2.0)
...
def test_dual_confirmation_logic():
    ...
    ai_signal = TradingSignal(
        action="BUY", confidence=0.8, pair="BTC/USDT", reason="Test"
    )
    should_enter = (
        ai_signal.action == "BUY"
        and ai_signal.confidence >= 0.5
        and (row["rsi"] < 70)
...
async def test_groq_api_fallback():
    ...
    with patch("pythia.application.ai_providers.groq_client.AsyncGroq") as mock_groq:
        ...
        with patch("pythia.application.ai_providers.groq_client.asyncio.sleep", new_callable=AsyncMock):
