...
class TestPriceValidator:
    ...
    def test_valid_prices(self):
        ...
        assert PriceValidator.validate(100.50) == Decimal("100.50")
...
class TestConcurrentTradeExecution:
    ...
    async def test_concurrent_trades_no_race_condition(self, db_session, test_portfolio):
        ...
        from app.core.enhanced_trading_engine_v2 import EnhancedTradingEngine
        ...
        assert test_portfolio.balance >= Decimal("10.0")  # MIN_BALANCE
        
        # Count successful trades
        successful = [r for r in results if r.get("success")]
        failed = [r for r in results if "error" in r]
        
        # Some should succeed, some should fail due to lock contention
    ...
    async def test_lock_timeout_immediate_failure(self, db_session, test_portfolio):
        ...
        from app.core.enhanced_trading_engine_v2 import atomic_trade_lock
        ...
        with atomic_trade_lock(db_session, test_portfolio.id):
            # Try to acquire same lock in second transaction
...
class TestStopLossAtomicity:
    ...
    def test_position_closure_creates_trade_first(self, db_session, test_position):
        ...
        from app.services.stop_loss_manager_v2 import StopLossTakeProfitManager
        ...
        trade = db_session.query(Trade).filter(
            Trade.symbol == test_position.symbol,
            Trade.side == "sell"
        ).first()
    ...
    def test_position_closure_rollback_on_error(self, db_session, test_position):
        ...
        # Verify position still open (rollback successful)
        db_session.refresh(test_position)
        assert test_position.status == "open"
        
        # Verify no orphaned trade record
        from app.db.models import Trade
        trade_count = db_session.query(Trade).filter(
            Trade.symbol == test_position.symbol,
            Trade.side == "sell"
        ).count()
