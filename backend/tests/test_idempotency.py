...
class TestCachedResponse:
    ...
    def test_not_expired_within_ttl(self):
        ...
        response = CachedResponse(status_code=200, body={'data': 'test'}, created_at=datetime.utcnow(), ttl_seconds=3600)
        assert not response.is_expired

    def test_expired_after_ttl(self):
        """Response should be expired after TTL."""
        response = CachedResponse(status_code=200, body={'data': 'test'}, created_at=datetime.utcnow() - timedelta(seconds=3700), ttl_seconds=3600)
...
class TestIdempotentDecorator:
    ...
    def test_payload_conflict_raises_error(self):
        ...
        with pytest.raises(IdempotencyConflictError):
            test_func(idem_key='key-conflict', value=10)
