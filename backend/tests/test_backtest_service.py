...
def test_execute_buy(backtest_engine):
    ...
    success = backtest_engine.execute_buy(timestamp=timestamp, symbol='AAPL', quantity=10, price=150.0)
...
def test_execute_sell(backtest_engine):
    ...
    success = backtest_engine.execute_sell(timestamp=timestamp, symbol='AAPL', quantity=5, price=160.0)
    assert success is True
