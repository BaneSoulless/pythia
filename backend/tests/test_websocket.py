...
class TestWebSocket:
    ...
    def test_portfolio_update_broadcast(self, client, db_session):
        ...
        with client.websocket_connect('/ws/portfolio/1') as websocket:
            # Message shape: {'type': 'portfolio_update', 'portfolio': {'id': 1, 'balance': 10500.0, 'total_value': 11200.0}}
    ...
    def test_trade_execution_notification(self, client):
        ...
        with client.websocket_connect('/ws/portfolio/1') as websocket:
            # Message shape: {'type': 'trade_executed', 'trade': {'symbol': 'AAPL', 'side': 'buy', 'quantity': 10, 'price': 150.0}}
            data = websocket.receive_json()
    ...
    def test_ai_status_update(self, client):
        ...
        with client.websocket_connect('/ws/portfolio/1') as websocket:
            # Message shape: {'type': 'ai_status_update', 'ai_status': {'is_training': True, 'epsilon': 0.1, 'total_episodes': 100}}
            data = websocket.receive_json()
            assert 'type' in data
