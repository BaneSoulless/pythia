...
class ErrorAnalyzer:
    ...
    def analyze_trade(self, trade: Trade) -> Dict:
        ...
        """
        analysis = {'trade_id': trade.id, 'success': False, 'error_type': None, 'lessons': []}
    ...
    def get_recent_performance(self, portfolio_id: int, days: int=7) -> Dict:
        """
        Analyze recent trading performance
        """
        cutoff_date = datetime.utcnow() - timedelta(days=days)
        recent_trades = self.db.query(Trade).filter(Trade.portfolio_id == portfolio_id, Trade.executed_at >= cutoff_date, Trade.side == 'sell').all()
        ...
        return {'total_trades': len(recent_trades), 'winning_trades': winning_trades, 'losing_trades': losing_trades, 'win_rate': winning_trades / len(recent_trades) if recent_trades else 0, 'total_pnl': total_pnl, 'avg_pnl': total_pnl / len(recent_trades)}

    def identify_patterns(self, portfolio_id: int) -> List[Dict]:
        """
        Identify patterns in trading behavior
        """
        trades = self.db.query(Trade).filter(Trade.portfolio_id == portfolio_id, Trade.side == 'sell').order_by(Trade.executed_at.desc()).limit(50).all()
        patterns = []
        time_performance = {}
        for trade in trades:
        ...
        if hour_avgs:
            ...
            patterns.append({'type': 'time_of_day', 'insight': f'Best trading hour: {best_hour}:00, Worst: {worst_hour}:00'})
        if any((t.ai_confidence for t in trades)):
            ...
            if high_conf_trades and low_conf_trades:
                ...
                patterns.append({'type': 'confidence_correlation', 'insight': f'High confidence avg P&L: {high_conf_pnl:.2f}€, Low: {low_conf_pnl:.2f}€'})
        return patterns

    def generate_learning_experience(self, state: Dict, action: Dict, reward: float, next_state: Dict, done: bool) -> LearningExperience:
        """
        Create a learning experience for the RL agent
        """
        experience = LearningExperience(state=state, action=action, reward=reward, next_state=next_state, done=done)
