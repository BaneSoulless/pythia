...
from pythia.infrastructure.persistence.models import LearningExperience, Trade
...
class ErrorAnalyzer:
            ...
            if trade.pnl > 0:
                ...
                if trade.ai_confidence and trade.ai_confidence < 0.5:
                    analysis["lessons"].append(
                        "Low confidence trade - consider raising threshold"
                    )
        return analysis
    ...
    def identify_patterns(self, portfolio_id: int) -> list[dict]:
        ...
        if any(t.ai_confidence for t in trades):
            high_conf_trades = [
                t for t in trades if t.ai_confidence and t.ai_confidence > 0.7
            ]
            low_conf_trades = [
                t for t in trades if t.ai_confidence and t.ai_confidence < 0.5
            ]
                ...
                patterns.append(
                    {
                        ...
                        "insight": f"High confidence avg P&L: {high_conf_pnl:.2f}€, Low: {low_conf_pnl:.2f}€",  # noqa: E501
                    }
                )
        return patterns
    ...
    def get_strategy_recommendations(self, portfolio_id: int) -> list[str]:
        ...
        if win_rate < 0.4:
            recommendations.append(
                "Win rate low (<40%) - Consider more conservative entry criteria"
            )
        elif win_rate > 0.6:
            recommendations.append(
                "Win rate good (>60%) - Current strategy is working well"
            )
        avg_pnl = perf.get("avg_pnl", 0)
        if avg_pnl < 0:
            recommendations.append(
                "Negative average P&L - Reduce position sizes or tighten stop-losses"
            )
        patterns = self.identify_patterns(portfolio_id)
        for pattern in patterns:
            if pattern["type"] == "confidence_correlation":
                recommendations.append(
                    "Consider filtering trades with AI confidence > 0.6"
                )
        return recommendations
