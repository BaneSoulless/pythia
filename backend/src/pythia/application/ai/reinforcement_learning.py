...
class StateBuilder:
    ...
    def build_state(portfolio_state: Dict, market_indicators: Dict, recent_trades: List) -> np.ndarray:
        ...
        state_features.extend([portfolio_state.get('balance', 10.0) / initial_balance, portfolio_state.get('total_value', 10.0) / initial_balance, portfolio_state.get('positions_count', 0) / 5.0])
        state_features.extend([market_indicators.get('rsi', 50) / 100.0, market_indicators.get('sma_20', 100) / 200.0, market_indicators.get('sma_50', 100) / 200.0 if market_indicators.get('sma_50') else 0.5])
...
class TradingRLAgent:
    ...
    def __init__(self, state_size: int=10, action_size: int=3, learning_rate: float=0.001, gamma: float=0.95, epsilon: float=1.0, epsilon_decay: float=0.995, epsilon_min: float=0.01, memory_size: int=10000):
    ...
    def _build_model(self) -> keras.Model:
        ...
        model = keras.Sequential([keras.layers.Dense(128, activation='relu', input_shape=(self.state_size,)), keras.layers.Dropout(0.2), keras.layers.Dense(128, activation='relu'), keras.layers.Dropout(0.2), keras.layers.Dense(64, activation='relu'), keras.layers.Dense(self.action_size, activation='linear')])
    ...
    def get_metrics(self) -> Dict:
        ...
        return {'epsilon': self.epsilon, 'memory_size': len(self.memory), 'avg_loss': np.mean(self.training_history['loss'][-100:]) if self.training_history['loss'] else 0, 'total_experiences': len(self.memory)}
...
class TradingEnvironment:
    ...
    def _get_state(self) -> np.ndarray:
        ...
        return np.array([current_data['close'], current_data.get('rsi', 50), self.balance, self.position])