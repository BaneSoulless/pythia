...
trades_total = Counter(
    "trading_bot_trades_total", "Total number of trades executed", ["side", "symbol"]
)
trade_pnl = Histogram(
    "trading_bot_trade_pnl",
    "Profit/Loss per trade",
    buckets=[-100, -50, -10, -5, 0, 5, 10, 50, 100, 500],
)
portfolio_value = Gauge(
    "trading_bot_portfolio_value", "Current portfolio total value", ["portfolio_id"]
)
portfolio_balance = Gauge(
    "trading_bot_portfolio_balance", "Current cash balance", ["portfolio_id"]
)
open_positions = Gauge(
    "trading_bot_open_positions", "Number of open positions", ["portfolio_id"]
)
ai_predictions_total = Counter(
    "trading_bot_ai_predictions_total", "Total AI predictions made", ["action"]
)
...
ai_training_duration = Summary(
    "trading_bot_ai_training_duration_seconds", "Time spent training AI model"
)
ai_epsilon = Gauge(
    "trading_bot_ai_epsilon", "Current epsilon (exploration rate)", ["model_id"]
)
...
api_request_duration = Histogram(
    "trading_bot_api_request_duration_seconds", "API request duration", ["endpoint"]
)
market_data_fetches_total = Counter(
    "trading_bot_market_data_fetches_total",
    "Total market data API calls",
    ["provider", "status"],
)
market_data_fetch_duration = Histogram(
    "trading_bot_market_data_fetch_duration_seconds",
    "Market data fetch duration",
    ["provider"],
)
errors_total = Counter(
    "trading_bot_errors_total", "Total errors encountered", ["error_type"]
)
risk_limit_violations = Counter(
    "trading_bot_risk_limit_violations_total",
    "Total risk limit violations",
    ["limit_type"],
)
min_balance_violations = Counter(
    "trading_bot_min_balance_violations_total", "Attempted trades below minimum balance"
)
...
def record_portfolio_metrics(
    portfolio_id: int, total_value: float, balance: float, positions_count: int
):
