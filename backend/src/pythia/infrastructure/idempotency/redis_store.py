...
class IdempotencyStore:
    ...
    def __init__(self, redis_url: str = "redis://localhost:6379/1"):
        ...
        self._set_if_not_exists = self.redis_client.register_script(
            LUA_IDEMPOTENCY_SCRIPT
        )
    ...
    async def check_and_set(self, idempotency_key: str, payload: dict) -> bool:
        ...
        result = await self._set_if_not_exists(
            keys=[key], args=[data, self.ttl_seconds]
        )
        return bool(result)

    async def get_response(self, idempotency_key: str) -> dict | None:
...
async def idempotency_middleware(request: Request, call_next):
    ...
    if request.method == "POST" and "/api/trades" in request.url.path:
        ...
        if not is_new:
            raise HTTPException(
                status_code=409, detail="Idempotent request previously completed"
            )
