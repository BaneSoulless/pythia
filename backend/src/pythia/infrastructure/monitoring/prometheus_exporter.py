...
markets_scanned_total = Counter(
    "pythia_markets_scanned_total", "Total prediction markets scanned", ["platform"]
)

# ML Meta-labeling Metrics
ml_trading_signals_total = Counter(
    "pythia_ml_trading_signals_total", "Total ML entry signals generated", ["symbol"]
)
ml_prediction_confidence = Gauge(
    "pythia_ml_prediction_confidence", "Confidence of ML model prediction", ["symbol"]
)
...
class MetricsExporter:
    ...
    def record_trade(self, platform: str, side: str, outcome: str):
        ...
        prediction_market_trades_total.labels(
            platform=platform, side=side, outcome=outcome
        ).inc()

    def record_order_latency(self, platform: str, latency_seconds: float):
        """Record order placement latency."""
        order_placement_latency_seconds.labels(platform=platform).observe(
            latency_seconds
        )
    ...
    def record_ml_signal(self, symbol: str, confidence: float):
        """Record ML signal and confidence."""
        ml_trading_signals_total.labels(symbol=symbol).inc()
