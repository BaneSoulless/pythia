...
class CCXTForexAdapter:
    ...
    def __init__(
        self, exchange_id: str = "oanda", config: dict[str, Any] | None = None
    ):
    ...
    async def create_order(
        ...
        if leverage > self.max_leverage:
            raise ValueError(
                f"Leverage {leverage} exceeds maximum allowed ({self.max_leverage})"
            )
    ...
    async def get_swap_rates(self, symbol: str) -> dict[str, float]:
        ...
        return {"long": long_swap, "short": short_swap}
