    ...
    class GroqClient:
        async def get_signal(self, *args, **kwargs) -> TradingSignal:
            return TradingSignal(
                action="HOLD", confidence=0.0, pair="test", reason="Mocked Provider"
            )
...
class FreqtradeStrategy(IStrategy):
    ...
    def emit_trade_event(self, pair: str, action: str, price: float, confidence: float):
        ...
        asyncio.run_coroutine_threadsafe(
            emit(EventType.TRADE_EXECUTED, data, source="freqtrade_adapter"), self._loop
        )
    ...
    def populate_indicators(
        ...
        dataframe["ema_fast"] = ta.trend.EMAIndicator(
            close=dataframe["close"], window=12
        ).ema_indicator()
        dataframe["ema_slow"] = ta.trend.EMAIndicator(
            close=dataframe["close"], window=26
        ).ema_indicator()
        return dataframe
    ...
    def populate_entry_trend(
        ...
        for idx, row in entered_trades.iterrows():
            ...
            payload = SignalPayload(
                ...
                base_signal=True,
                ml_confidence=Decimal(str(row["ml_confidence"])),
