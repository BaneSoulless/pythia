...
# Load P1 Modules if available, fallback per Test Mode
try:
    from app.domain.cognitive.models import TradingSignal
except ImportError:
    # Dummy import fallback in standalone run
...
try:
    from app.services.ai_providers.groq_client import GroqClient
except ImportError:
    class GroqClient:
...
class FreqtradeStrategy(IStrategy):
    ...
    def get_ai_signal(self, pair: str, price: float, rsi: float, ema_fast: float, ema_slow: float) -> TradingSignal:
        ...
        future = asyncio.run_coroutine_threadsafe(
            self.groq_client.get_signal(pair, price, rsi, ema_fast, ema_slow),
            self._loop
        )
        try:
    ...
    def populate_indicators(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        import ta
        # Technical analysis calculations (Example: RSI and EMAs)
    ...
    def populate_entry_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        for i in range(len(dataframe)):
            ...
            ai_signal = self.get_ai_signal(
                ...
                dataframe.iloc[i]['rsi'],
                dataframe.iloc[i]['ema_fast'],
                dataframe.iloc[i]['ema_slow']
            )
            
            # Injection AI params down to Dataframe per Streamlit logging o analisi
            dataframe.at[i, 'ai_confidence'] = ai_signal.confidence
            
            # Il vincolo confidence >= 0.5 è già imposto in Pydantic "Confidence Gate" (TradingSignal)
            # Belt-and-suspenders validation: Technical Indicators Filter
    ...
    def populate_exit_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        for i in range(len(dataframe)):
             if pd.isna(dataframe.iloc[i]['rsi']): continue
             ...
             if ai_signal.action == "SELL" and ai_signal.confidence >= 0.7:  # Higher threshold to sell
