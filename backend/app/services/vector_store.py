...
class VectorStoreService:
    ...
    def __init__(self, mode: str = "local"):
        ...
        if self.mode == "production":
            ...
            # Si connette a ChromaDB in locale
    ...
    async def add_texts(self, texts: List[str], metadatas: List[Dict[str, Any]], ids: List[str]) -> None:
        ...
        if self.mode == "production":
            # Logica sincrona Weaviate (Weaviate client v4)
            collection = self.client.collections.get(self.collection_name)
            with collection.batch.dynamic() as batch:
                for idx, text in enumerate(texts):
                    batch.add_object(
                        properties={"text": text, **metadatas[idx]},
                        uuid=ids[idx]
                    )
        else:
            self.collection.add(
    ...
    async def similarity_search(self, query: str, limit: int = 5) -> List[SearchResult]:
        ...
        if self.mode == "production":
            ...
            for obj in response.objects:
                # In Weaviate v4, la distanza cosine è solitamente fra 0 e 2, similarity = 1 - distance/2
                # Per cosine similarity threshold, gestiamo le conversioni.
                distance = obj.metadata.distance if obj.metadata.distance is not None else 1.0
                score = 1.0 - distance
                if score >= self.similarity_threshold:
                    results.append(SearchResult(
                        chunk=obj.properties.get("text", ""),
