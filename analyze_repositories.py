...
class RepositoryAnalyzer:
        ...
        for py_file in self.repo_path.rglob("*.py"):
            ...
            try:
                with open(py_file, 'r', encoding='utf-8') as f:
                    content = f.read()
                    self.analysis["files"][str(py_file.relative_to(self.repo_path))] = {
    ...
    def extract_patterns(self):
        ...
        patterns = {
            ...
            "event_sourcing": ["EventStore", "event_log", "append_event"],
            "circuit_breaker": ["CircuitBreaker", "circuit_breaker"],
            "cqrs": ["CommandHandler", "QueryHandler", "command_bus"],
            "strategy_pattern": ["Strategy", "IStrategy", "BaseStrategy"],
            "factory_pattern": ["Factory", "create_", "builder"],
            "observer_pattern": ["Observer", "subscribe", "notify"],
            "singleton": ["Singleton", "_instance"],
            "dependency_injection": ["inject", "Inject", "Container"],
            "repository_pattern": ["Repository", "repo"],
            "orm": ["SQLAlchemy", "Model", "Column"],
            "caching": ["cache", "Cache", "redis", "Redis"],
            "logging": ["logger", "logging", "log_"],
            "testing": ["pytest", "unittest", "mock"],
            "api": ["FastAPI", "Flask", "router", "endpoint"],
            "websocket": ["WebSocket", "ws", "socket"],
            "ml": ["tensorflow", "torch", "sklearn", "model"],
            "rl": ["gym", "agent", "reward", "policy"]
        }
    ...
    def analyze_architecture(self):
        ...
        for dir_name, count in dir_counts.items():
            if count > 5:  # Significant modules
    ...
    def extract_dependencies(self):
        ...
        external = {imp.split('.')[0] for imp in all_imports 
                   if not imp.startswith(self.repo_path.name)}
    ...
    def calculate_metrics(self):
        ...
        self.analysis["metrics"] = {
            ...
            "avg_lines_per_file": total_lines / len(self.analysis["files"]) if self.analysis["files"] else 0,
            "avg_functions_per_file": total_functions / len(self.analysis["files"]) if self.analysis["files"] else 0
        }
...
def generate_comparison_report(results: Dict):
    ...
    for name, data in results.items():
        m = data["metrics"]
        report.append(f"| {name} | {m['total_files']} | {m['total_lines']:,} | "
                     f"{m['total_classes']} | {m['total_functions']} | "
                     f"{m['avg_lines_per_file']:.1f} |\n")
...
def extract_implementation_patterns(results: Dict):
    ...
    for name, data in results.items():
        patterns[name] = {
            "has_async": len(data["patterns"].get("async", [])) > 0,
            "has_event_sourcing": len(data["patterns"].get("event_sourcing", [])) > 0,
            "has_cqrs": len(data["patterns"].get("cqrs", [])) > 0,
            "has_circuit_breaker": len(data["patterns"].get("circuit_breaker", [])) > 0,
            "has_caching": len(data["patterns"].get("caching", [])) > 0,
            "has_websocket": len(data["patterns"].get("websocket", [])) > 0,
            "has_ml": len(data["patterns"].get("ml", [])) > 0,
            "has_rl": len(data["patterns"].get("rl", [])) > 0,
            "strategy_files": data["patterns"].get("strategy_pattern", []),
            "api_files": data["patterns"].get("api", []),
            "orm_files": data["patterns"].get("orm", [])
        }
