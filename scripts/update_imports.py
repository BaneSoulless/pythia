...
def rewrite_imports_in_file(filepath: Path):
    ...
    if "app" not in source:
        return # Skip if obvious no match
    ...
    # ast.unparse è disponibile in Python 3.9+
...
def main():
    ...
    # 1. Aggiorna i file in src/pythia/
    if src_pythia.exists():
        for py_file in src_pythia.rglob("*.py"):
            rewrite_imports_in_file(py_file)
            
    # 2. Aggiorna i file in tests/
    if tests_dir.exists():
        for py_file in tests_dir.rglob("*.py"):
            rewrite_imports_in_file(py_file)
            
    # 3. Aggiorna backend/main.py se rootato fuori da src
    main_py = backend_dir / "main.py"
    if main_py.exists():
        rewrite_imports_in_file(main_py)
         
    # 4. Aggiorna adapter e script root
    ...
    # 5. Aggiorna Dockerfile e docker-compose.yml
